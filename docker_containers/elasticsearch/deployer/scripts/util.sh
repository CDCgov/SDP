...
function generate_JKS_chain() {
    ...
    "$keytool" -genkey \
        ...
        -keysize   2048 \
        -validity  712 \
        -dname "CN=$NODE_NAME, OU=OpenShift, O=ElasticSearch" \
...
function generate_PEM_cert() {
    ...
    openssl req -out "$dir/$NODE_NAME.csr" -new -newkey rsa:2048 -keyout "$dir/$NODE_NAME.key" -subj "/CN=$NODE_NAME/OU=OpenShift/O=ElasticSearch" -days 712 -nodes
...
function get_es_dcs() {
    oc get dc --selector es-infra=elasticsearch -o name
}

function get_curator_dcs() {
    oc get dc --selector es-infra=curator -o name
}
...
# $1 - kibana pod name
# $2 - es hostname (e.g. es-es or es-es-ops)
# $3 - project name (e.g. es, test, .operations, etc.)
# $4 - _count or _search
# $5 - field to search
# $6 - search string
...
# $1 - es pod name
# $2 - es hostname (e.g. es-es or es-es-ops)
# $3 - index name (e.g. project.es, project.test, .operations, etc.)
# $4 - _count or _search
# $5 - field to search
# $6 - search string
# stdout is the JSON output from Elasticsearch
...
function wait_for_fluentd_to_catch_up() {
    ...
    if espod=$es_pod myproject=project.es mymessage=$uuid_es expected=$expected \
            wait_until_cmd_or_err test_count_expected test_count_err $timeout ; then
        echo good - $FUNCNAME: found $expected record project es for $uuid_es
    else
        echo failed - $FUNCNAME: not found $expected record project es for $uuid_es after $timeout seconds
