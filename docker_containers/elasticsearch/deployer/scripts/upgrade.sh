...
TIMES=300
fluentd_nodeselector="es-infra-fluentd=true"
...
function getDeploymentVersion() {
  ...
  if [[ ( -z "$(oc get daemonset -l es-infra=fluentd -o jsonpath='{.items[*].metadata.name}')" && -z "$(oc get template es-fluentd-template -o yaml | grep 'kind: DaemonSet')" ) || -z "$(oc get dc -l es-infra=curator -o jsonpath='{.items[*].metadata.name}')" ]]; then
    echo 1
    return
  fi

  # Check for DC triggers
  if [[ -n "$(oc get dc -l es-infra -o jsonpath='{.items[?(@.spec.triggers[*].type)].metadata.name}')" ]]; then
    echo 2
    return
  fi

  # check for configmaps for ES and curator
  if [[ -z "$(oc get configmap/elasticsearch)" || -z "$(oc get configmap/es-curator)" ]]; then
    echo 3
    return
  fi

  # check for configmap for fluentd
  if [[ -z "$(oc get configmap/es-fluentd)" ]]; then
    echo 4
    return
  fi

  # check for ES 2.3 configmap and NAMESPACE downward API env var
  if [[ -n "$(oc get configmap/elasticsearch -o yaml | grep -a1 'openshift:' | grep 'acl:')" || -z "$(oc get dc -l es-infra=es -o yaml | grep 'NAMESPACE')" ]]; then
    echo 5
    return
  fi

  # check for common data model
  if [[ -z "$(oc get configmap/elasticsearch -o yaml | grep 'use_common_data_model: true')" ]]; then
    echo 6
    return
  fi
...
function scaleDownDCsAndWait() {
  ...
  local dcs=$(oc get dc -l es-infra=$infra -o jsonpath='{.items[*].metadata.name}')
  for dc in $dcs; do
...
function scaleDown() {
  ...
  local fluentd_dc=(`oc get dc -l es-infra=fluentd -o jsonpath='{.items[*].metadata.name}'`)

  if [[ -z "$fluentd_dc" ]]; then
    local selector=$(oc get daemonset -l es-infra=fluentd -o jsonpath='{.items[*].spec.template.spec.nodeSelector}')
    if [[ -n "$selector" ]]; then
...
function scaleUpDCsAndCheck() {
  ...
  local dcs=$(oc get dc -l es-infra=$component -o jsonpath='{.items[*].metadata.name}')
...
# Fluentd
  ...
  if [[ -z "$fluentd_dc" ]]; then
    if [[ -z $(oc get daemonset -l es-infra=fluentd -o jsonpath='{.items[*].metadata.name}') ]]; then
      # if the daemonset doesn't exist, recreate it here
      if [[ -z $(oc get template -l es-infra=fluentd -o jsonpath='{.items[*].metadata.name}') ]]; then
...
function patchDCImage() {
  ...
  if [ "$kibana" = true ]; then
    authProxy_patch="{.spec.template.spec.containers[1].image}=${IMAGE_PREFIX}es-auth-proxy:${IMAGE_VERSION}"
  fi
...
function updateImages() {
  ...
  patchTemplateParameter "es-imagestream-template"

  # patch all es
  patchImageVersion "es-infra=elasticsearch" "elasticsearch"

  # patch all Kibana
  patchImageVersion "es-infra=kibana" "es-kibana" true

  # patch all curator
  patchImageVersion "es-infra=curator" "es-curator"

  # patch fluentd -- or just recreate template?
  patchImageVersion "es-infra=fluentd" "es-fluentd" false true
...
function add_curator() {
  ...
  oc delete dc,rc,template --selector es-infra=curator || :

  generate_curator_template
  generate_curator

  for dc in $(oc get dc -l es-infra=curator -o name); do
    oc deploy $dc --latest
  done
}
...
function add_fluentd_daemonset() {
  ...
  # this may fail otherwise if there was only a partial upgrade previously
  oc delete dc,rc,template,daemonset --selector es-infra=fluentd || :
...
function add_config_maps() {
  ...
  local dc patch=$(join , \
    ...
    '{"op": "add", "path": "/spec/template/spec/volumes/1", "value": {"name": "elasticsearch-config", "configMap": {"name": "elasticsearch"}}}' \
  )
  ...
  patch=$(join , \
    ...
    '{"op": "add", "path": "/spec/template/spec/volumes/1", "value": {"name": "config", "configMap": {"name": "es-curator"}}}' \
  )
...
function add_fluentd_configmaps() {
  ...
  oc label configmap/es-fluentd es-infra=support
}
...
function regenerate_config_and_support_objects() {
  oc process es-support-template | oc delete -f - || :
  oc delete service,route,template --selector es-infra=support
...
function remove_triggers_and_IS() {
  ...
  for dc in $(oc get dc -l es-infra -o jsonpath='{.items[?(@.spec.triggers[*].type)].metadata.name}'); do
  ...
  oc delete is -l es-infra=support
}

# at some point we dropped the separate-but-equal kibana secrets
function update_kibana_ops_proxy_secret() {
  local dc
  for dc in $(oc get dc -o name -l component=kibana-ops,es-infra=kibana); do
    oc set volume $dc --add --overwrite --name kibana-proxy \
                      --type secret --secret-name es-kibana-proxy
    oc deploy --latest $dc
  done
  oc delete secret es-kibana-ops-proxy || :
}
