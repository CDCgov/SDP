...
function delete_es() {
  ...
  echo "Deleting all other es objects"
  oc process es-support-template | oc delete -f - || :
  oc delete dc,rc,svc,routes,templates,daemonset --selector es-infra
  oc delete is -l es-infra=support
  # secrets don't have label selectors
  oc delete secret es-fluentd elasticsearch es-kibana es-kibana-proxy es-curator es-curator-ops || :
}
...
function initialize_install_vars() {
  ...
  if configmap=$(oc get configmap/es-deployer --template="$index_template"); then
    for index in $configmap; do
      input_vars[$index]=$(oc get configmap/es-deployer --template="{{println (index .data \"$index\")}}")
    done
  fi
  # if secret exists, get values from it
  if secret=$(oc get secret/es-deployer --template="$index_template"); then
    for index in $secret; do
      : ${input_vars[$index]:=$(oc get secret/es-deployer --template="{{println (index .data \"$index\")}}" | base64 -d)}
    done
  fi
  # if legacy variables set, use them to fill unset inputs
  for var in PUBLIC_MASTER_URL IMAGE_PULL_SECRET ES_NODESELECTOR \
             ES_{INSTANCE_RAM,PVC_SIZE,PVC_PREFIX,PVC_DYNAMIC,CLUSTER_SIZE,NODE_QUORUM,RECOVER_AFTER_NODES,RECOVER_EXPECTED_NODES,RECOVER_AFTER_TIME}
  do
    [ ${!var+set} ] || continue
    index=${var,,} # lowercase
...
function generate_support_objects() {
  ...
  oc new-app es-support-template
...
#  fi
#  oc create route reencrypt --service="es-kibana" \
#                             --hostname="${hostname}" \
...
#  fi
#  oc create route reencrypt --service="es-kibana-ops" \
...
function generate_signer_cert_and_conf() {
  ...
  if [ ${input_vars[ca.key]+set} ]; then
    ...
    openshift admin ca create-signer-cert  \
      ...
      --name="es-signer-$(date +%Y%m%d%H%M%S)"
  fi
...
#    fluentd_user='system.es.fluentd'
#    kibana_user='system.es.kibana'
#    curator_user='system.es.curator'
...
#    generate_PEM_cert "$curator_user"
    ...
    generate_JKS_chain true elasticsearch "es"
    ...
    generate_JKS_chain false es "$(join , es{,-cluster}{,.${project}.svc.cluster.local})"
...
#    oc secrets new es-kibana \
#        ca=$dir/ca.crt \
#        key=$dir/${kibana_user}.key cert=$dir/${kibana_user}.crt
#    oc secrets new es-kibana-proxy \
...
#    oc secrets new es-fluentd \
#        ca=$dir/ca.crt \
#        key=$dir/${fluentd_user}.key cert=$dir/${fluentd_user}.crt
#    oc secrets new es-curator \
#        ca=$dir/ca.crt \
#        key=$dir/${curator_user}.key cert=$dir/${curator_user}.crt
#    oc secrets new es-curator-ops \
#        ca=$dir/ca.crt \
#        key=$dir/${curator_user}.key cert=$dir/${curator_user}.crt
    echo "Attaching secrets to service accounts"
#    oc secrets add serviceaccount/aggregated-es-kibana \
#                   es-kibana es-kibana-proxy
    oc secrets add serviceaccount/elasticsearch \
                   elasticsearch
#    oc secrets add serviceaccount/aggregated-es-fluentd \
#                   es-fluentd
#    oc secrets add serviceaccount/aggregated-es-curator \
#                   es-curator
    if [ -n "${input_vars[image-pull-secret]}" ]; then
      for account in default elasticsearch; do
...
function generate_configmaps() {
    ...
    oc delete configmap -l es-infra=support
...
#    oc create configmap es-curator \
#      --from-file=config.yaml=conf/curator.yml
#    oc label configmap/es-curator es-infra=support
...
function generate_curator() {
  oc new-app es-curator-template
  if [ "${input_vars[enable-ops-cluster]}" == true ]; then
    oc new-app es-curator-ops-template
  fi
}

function generate_kibana() {
  oc new-app es-kibana-template
  if [ "${input_vars[enable-ops-cluster]}" == true ]; then
    oc new-app es-kibana-ops-template
  fi
}

function generate_fluentd() {
  oc new-app es-fluentd-template
}
  ...
  if [ "${input_vars[enable-ops-cluster]}" == true ]; then
    ...
    for ((n=1;n<=${es_ops_cluster_size};n++)); do
      ...
      if [ "${pvcs[$pvc]}" != 1 -a "${es_ops_pvc_size}" != "" ]; then # doesn't exist, create it
        oc new-app es-pvc-${es_ops_dynamic}template \
           --param "NAME=$pvc" --param "SIZE=${es_ops_pvc_size}"
        pvcs["$pvc"]=1
      fi
      if [ "${pvcs[$pvc]}" = 1 ]; then # exists (now), attach it
            oc process es-es-ops-template | \
              oc volume -f - \
                  --add --overwrite --name=elasticsearch-storage \
                  --type=persistentVolumeClaim --claim-name="$pvc" -o yaml | \
              oc create -f -
      else
            oc new-app es-es-ops-template
      fi
    done
  fi
}
...
#  generate_fluentd
  ...
  for dc in $(oc get dc -l es-infra -o name); do
    oc deploy $dc --latest
  done
} #generate_objects()
...
function notify_user() {
  set +x
  echo 'Success!'
#  fns=${input_vars[fluentd_nodeselector]:-es-infra-fluentd=true}
#  ops_cluster_section=""
#  if [ "${input_vars[enable-ops-cluster]}" == true ]; then
#    ops_cluster_section="
#Operations logs:
...
required for elasticsearch. You now have a few more steps to run manually.
