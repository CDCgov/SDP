...
ARTIFACT_DIR=${ARTIFACT_DIR:-${TMPDIR:-/tmp}/origin-aggregated-es}
...
function createOldIndexPattern() {
  ...
  oc exec $esPod -- curl -s --cacert /etc/elasticsearch/secret/admin-ca --cert /etc/elasticsearch/secret/admin-cert --key /etc/elasticsearch/secret/admin-key \
                         -XPUT "https://es-es:9200/oldindex.${genUUID}.${indexDate}" -d '{ "settings": { "index": { "number_of_shards": 1, "number_of_replicas": 0 } } }'

  # necessary?
  oc exec $esPod -- curl -s --cacert /etc/elasticsearch/secret/admin-ca --cert /etc/elasticsearch/secret/admin-cert --key /etc/elasticsearch/secret/admin-key \
                         -XGET "https://es-es:9200/_cluster/health/oldindex.${genUUID}.${indexDate}?wait_for_status=yellow&timeout=50s"
}
...
function deleteOldIndexPattern() {
  ...
  esPod=`oc get pods -l component=es -o name | sed "s,pod/,,"`
  oc exec $esPod -- curl -s --cacert /etc/elasticsearch/secret/admin-ca --cert /etc/elasticsearch/secret/admin-cert --key /etc/elasticsearch/secret/admin-key \
     -XDELETE "https://es-es:9200/oldindex.*"
}
...
function removeFluentdConfigMaps() {
  ...
  oc patch template/es-fluentd-template --type=json --patch "[$patch]" || return 1
  oc delete configmap/es-fluentd || return 1
  return 0
}
...
function removeEsCuratorConfigMaps() {
  ...
  oc delete configmap/es-elasticsearch configmap/es-curator || return 1
...
function removeCurator() {
  ...
  for curator_dc in $(oc get dc -l es-infra=curator -o jsonpath='{.items[*].metadata.name}'); do
...
function useFluentdDC() {
  ...
  oc scale dc es-fluentd --replicas=1
  oc deploy dc/es-fluentd --latest || :
...
function removeAdminCert() {
  ...
  oc patch secret es-elasticsearch -p '{"data":{"admin-cert": null}}'
...
function addTriggers() {
  ...
  # the upgrade script looks for
  # oc get dc -l es-infra -o jsonpath='{.items[?(@.spec.triggers[*].type)].metadata.name}'
  # to exist

  for dc in $(oc get dc -l es-infra -o jsonpath='{.items[*].metadata.name}'); do
...
function rebuildVersion() {
  ...
  for bc in $(oc get bc -l es-infra -o jsonpath='{.items[*].metadata.name}'); do
...
function upgrade() {
  ...
  UPGRADE_POD=$(get_latest_pod "es-infra=deployer")
...
### check templates and DC patched
  for template in $(oc get template -l es-infra -o name); do
  ...
  for image in $(oc get dc -l es-infra -o jsonpath='{.items[*].spec.template.spec.containers[*].image}'); do
...
### check for admin-cert, admin-key, admin-ca
  [[ -z "$(oc get secret/es-elasticsearch -o jsonpath='{.data.admin-ca}')" ]] && return 1
  [[ -z "$(oc get secret/es-elasticsearch -o jsonpath='{.data.admin-key}')" ]] && return 1
  [[ -z "$(oc get secret/es-elasticsearch -o jsonpath='{.data.admin-cert}')" ]] && return 1
...
### check for Fluentd daemonset, no DC exists
  [[ -z "$(oc get daemonset/es-fluentd -o name)" ]] && return 1
  [[ -n "$(oc get dc -l es-infra=fluentd -o name)" ]] && return 1

### check for Curator
  [[ -z "$(oc get dc -l es-infra=curator -o name)" ]] && return 1

### check for no triggers in DC
  [[ -n "$(oc get dc -l es-infra -o jsonpath='{.items[?(@.spec.triggers[*].type)].metadata.name}')" ]] && return 1
...
# this is treated differently than how it is in es.sh -- set it to be in seconds
TIME_MIN=60
