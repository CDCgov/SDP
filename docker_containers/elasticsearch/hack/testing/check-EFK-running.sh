...
if [[ "$CLUSTER" == "true" ]]; then
  NEEDED_COMPONENTS=("es-es-[a-ZA-Z0-9]+?0" "es-kibana0" "es-curator0" "es-es-ops-[a-zA-Z0-9]+?0" "es-kibana-ops0" "es-curator-ops0")
else
  NEEDED_COMPONENTS=("es-es-[a-zA-Z0-9]+?0" "es-kibana0" "es-curator0")
fi
...
FOUND_DC=(`oc get dc -l es-infra -o jsonpath='{.items[*].metadata.labels.component}' | xargs -n1 | sort -u | xargs`)
DC_COUNT=${#FOUND_DC[@]}
...
FOUND_RC=(`oc get rc -l es-infra -o jsonpath='{.items[*].metadata.labels.component}' | xargs -n1 | sort -u | xargs`)
RC_COUNT=${#FOUND_RC[@]}
...
if [ -n "${ES_HOST:-}" ] ; then
    NEEDED_ROUTES=(${NEEDED_ROUTES[@]} "es-es")
fi
if [ -n "${ES_OPS_HOST:-}" ] ; then
    NEEDED_ROUTES=(${NEEDED_ROUTES[@]} "es-es-ops")
fi
FOUND_ROUTES=(`oc get routes -l es-infra=support -o jsonpath='{.items[*].metadata.name}'`)
...
if [[ $ROUTE_COUNT -ne $NEEDED_ROUTE_COUNT ]]; then
  ...
  echo "* Please rerun \`oc process es-support-template | oc create -f -\` to generate missing routes."
else
...
NEEDED_SERVICE=("es-es0" "es-es-cluster0" "es-es-ops0" "es-es-ops-cluster0" "es-kibana0" "es-kibana-ops0")
FOUND_SERVICE=(`oc get svc -l es-infra=support -o jsonpath='{.items[*].metadata.name}'`)
...
FOUND_OAUTH=(`oc get oauthclient -l es-infra=support -o jsonpath='{.items[*].metadata.name}'`)
...
if [[ $OAUTH_COUNT -ne $NEEDED_OAUTH_COUNT ]]; then
  ...
  echo "* Please rerun \`oc process es-support-template | oc create -f -\` to generate missing oauth client."
else
...
FOUND_DAEMONSET=(`oc get daemonset -l es-infra=fluentd -o jsonpath='{.items[*].metadata.name}'`)
...
if [[ $DAEMONSET_COUNT -ne $NEEDED_DAEMONSET_COUNT ]]; then
  ...
  echo " ! daemonset es-fluentd is missing..."
  EXIT_CODE=1

  echo "* Please rerun \`oc process es-fluentd-template | oc create -f -\` to generate missing daemonset."
else
...
NEEDED_PODS=("${NEEDED_COMPONENTS[@]}" es-fluentd)
