...
cleanup_forward() {
  ...
  oc delete daemonset/es-forward-fluentd || :

  # Revert configmap if we haven't yet
  if [ -n "$(oc get configmap/es-fluentd -o yaml | grep '<match \*\*>')" ]; then
    oc get configmap/es-fluentd -o yaml | sed -e '/<match \*\*>/ d' \
        -e '/@include configs\.d\/user\/secure-forward\.conf/ d' \
        -e '/<\/match>/ d' | oc replace -f -
  fi

  oc patch configmap/es-fluentd --type=json --patch '[{ "op": "replace", "path": "/data/secure-forward.conf", "value": "\
# @type secure_forward\n\
# self_hostname forwarding-${HOSTNAME}\n\
# shared_key aggregated_es\n\
#  secure no\n\
#  <server>\n\
...
update_current_fluentd() {
  ...
  # undeploy fluentd
  oc label node --all es-infra-fluentd-

  wait_for_pod_ACTION stop $fpod

  # edit so we don't send to ES
  oc get configmap/es-fluentd -o yaml | sed '/## matches/ a\
      <match **>\
...
create_forwarding_fluentd() {
 # create forwarding configmap named "es-forward-fluentd"
 oc create configmap es-forward-fluentd \
    --from-file=fluent.conf=../templates/forward-fluent.conf

 # create forwarding daemonset
  oc get template/es-fluentd-template -o yaml | \
    sed -e 's/es-infra-fluentd: "true"/es-infra-forward-fluentd: "true"/' \
        -e 's/name: es-fluentd/name: es-forward-fluentd/' \
  ...
  oc label node --all es-infra-forward-fluentd=true
